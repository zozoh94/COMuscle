...
public class Main {
	...
	public static void main(String[] args) {
		...
		Segment segment1 = new Segment(10, true); // true = crÃ©e
													// automatiquement 2
													// nouvelles extrÃ©mitÃ©s
		segment1.setFiles(new ArrayList<File>(Arrays.asList(file1, file2))); // On
																				// oublit
																				// pas
																				// d'ajouter
																				// les
		...
		segment2.setFiles(new ArrayList<File>(Arrays.asList(file1, file2))); // Partage
																				// les
																				// mÃªmes
																				// files
																				// que
		...
		Segment segment3 = new Segment(10, true); // true = crÃ©e
													// automatiquement 2
													// nouvelles extrÃ©mitÃ©s
		segment3.setFiles(new ArrayList<File>(Arrays.asList(file1, file2)));

		Segment segment4 = new Segment(10, true); // true = crÃ©e
													// automatiquement 2
													// nouvelles extrÃ©mitÃ©s
		...
		// Jonction va se lier Ã segment1.extremit2 et segment2.extremite1 par
		// dÃ©faut, si besoin d'autres extremitÃ©s il faut les spÃ©cifier
		// explicitement
		// /!\ on ajoute pas de files ici car elles sont automatiquement
		// ajoutÃ©es en rÃ©cupÃ©rant les infos de segment1 et segment2 (voir
		...
		while (continuer) {
			...
			try {
					...
					for (int i=0; i<carrefour.getExtremites().size(); i++)
					if (voiture1.getPosition().getPosition()==voiture1.getPosition().getEmplacement().longueur) {
						voiture1.setPosition(new Position(carrefour.getExtremites().get(i).getSonSegment()));
						voiture1.deplacer();
					}
					voiture1.deplacer();	
			 ...
			 catch (DemiTourImpossibleException e1) {
